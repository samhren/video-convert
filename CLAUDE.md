...
- **Completion state is a set of the EXPECTED chunk indexes, never a counter.** SQS is at-least-once; the same chunk can ping "done" more than once. `done.add(i)` is idempotent; a counter would over-count on redelivery and trigger reassembly before all distinct chunks exist. The DO is initialized with `N` and `/done` only accepts indexes in `[0, N)`, so unknown ids can't be counted and `done.size` can never exceed `N`. Completion is **latched** via the status state machine — never recomputed as `done.size === N` on read, or a late/duplicate ping could flip it back.
...
- **Reassembly is order-independent and fires exactly once on success.** Workers finish out of order — reassembly sorts by chunk index, not arrival. A persisted status state machine (`pending → reassembling → done | failed`) guards it: the terminal `done` state prevents a double-fire, while a crash mid-reassembly leaves a retryable `reassembling`/`failed` state rather than a falsely-completed job. The status is persisted *before* reassembly runs, and reassembly output must be idempotent so a retry is safe.
